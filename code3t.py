...
# AHI4% prioritized patterns
AHI4_PATTERNS = [
    re.compile(r'\bAHI4%\s*:\s*(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bpAHI4%\s*:\s*(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bTotal\s+AHI4%\s*:\s*(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bAHI4%\s*=\s*(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bAHI4%\s+(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bAHI\s*4%\s+(?P<val>\d+(?:\.\d+)?)(?:/hr)?', re.IGNORECASE),
    re.compile(r'\bAHI\s*\(4%\)\s+(?P<val>\d+(?:\.\d+)?)(?:/hr)?', re.IGNORECASE),
    re.compile(r'AHI\s*3%\s*\d+(?:\.\d+)?/4%\s+(?P<val>\d+(?:\.\d+)?)', re.IGNORECASE),
    re.compile(r'\bAHI\s*4%\s+of\s+(?P<val>\d+(?:\.\d+)?)(?:/hour)?', re.IGNORECASE),
]

# General AHI patterns
AHI_PATTERNS = [
...
# -------------------------
# Collect *all* AHI/4% matches in textual order (RDI REMOVED)
# -------------------------
def extract_all_ahi_matches(narrative: str):
    """
    Returns a list of dicts:
        [{'kind': 'AHI'|'AHI4', 'value': float, 'phrase': str, 'start': int}, ...]
    ...
    pattern_specs = []
    for pat in AHI4_PATTERNS:
        pattern_specs.append(('AHI4', pat))
...
def process_file(input_path: str, output_path: str = "KPSC_TOTAL.csv"):
    ...
    # Expand AHI4_vals into AHI4%-1, AHI4%-2, ...
    max_len_ahi4 = result['AHI4_vals'].apply(lambda x: len(x) if isinstance(x, list) else 0).max()
    max_len_ahi4 = int(max_len_ahi4) if pd.notnull(max_len_ahi4) else 0
    for j in range(max_len_ahi4):
        col = f'AHI4%-{j+1}'
        result[col] = result['AHI4_vals'].apply(lambda x: (x[j] if isinstance(x, list) and len(x) > j else None))

    # Final column order (note: no 'Total AHI')
    cols_base = ['MRN', 'PROC_DATE', 'CODE']
    cols_ahi  = [f'AHI-{i+1}' for i in range(max_len_ahi)]
    cols_ahi4 = [f'AHI4%-{j+1}' for j in range(max_len_ahi4)]
    cols_tail = ['ESS', 'AHI_bool', 'ESS_bool', 'NARRATIVE_AHI', 'NARRATIVE_ESS']  # <<< NEW order w/ flags

    keep_cols = [c for c in cols_base + cols_ahi + cols_ahi4 + cols_tail if c in result.columns]
    result = result[keep_cols]


    # Clean up temp lists
    result = result.drop(columns=['AHI_vals', 'AHI4_vals'], errors='ignore')
